(function (window) {
    ...
    if (window.jsc_AJAX_ModalWebForm) return;

    window.jsc_AJAX_ModalWebForm = function (arParams) {
        this.scriptUrl = '';
        this.ajaxUrl = '';
        this.modalId = '';
        ...
        if (this.errorCode === 0) {
            ...
            console.log('jsc_AJAX_ModalWebForm errorCode =', this.errorCode);
        }
    };

    window.jsc_AJAX_ModalWebForm.prototype = {
        init: function () {
            let this_ = this;

            $('body').on('click', '.js-form-modal', function (event) {
