...
$formCssId = \Baza23\AuthForms::AF_FORM_CSS_ID . '--' . $arAFParams["CODE"];
...
if (empty($uParams)
    ...
    $ibSectionCode = ($_REQUEST["ib_section_code"] ?
            $_REQUEST["ib_section_code"] : $arAFParams["IBLOCK_FORM_SECTION_CODE"]);
    if ($ibSectionCode) {
    ...
    $dataPhp = false;
    if ($templateFolder) {
        $dataPhp = $templateFolder . '/auth-form.php';
    ...
    $errorPhp = str_replace('/auth-form.php', '/errors.php', $dataPhp);
    ...
    $uParams = array(
        ...
        "TEMPLATE_NAME" => $_REQUEST["template_name"] ?
                $_REQUEST["template_name"] : $arAFParams["TEMPLATE_NAME"],
